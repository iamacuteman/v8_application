...
export const useSystemStore = defineStore({
    ...
    state: () =>
        localStorage.getItem('systemConfig') ? JSON.parse(localStorage.getItem('systemConfig')) :
            {
                sideBar: {
                    // 是否折叠菜单
                    isCollapse: false
    ...
    actions: {
        ...
        addTab(tabItem) {
            ...
            if (!this.tabList.some(item => item.fullPath === tabItem.fullPath)) {
                if(tabItem.fullPath.indexOf('active_key') === -1){
                    tabItem.fullPath += '?active_key=' + active_key
                }
